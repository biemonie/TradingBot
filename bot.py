...
# ============================================================
def check_1h_candle_confirmation(symbol, signal_type):
    """
    ...
    """
    data = get_ohlcv(symbol, interval="1h", bars=5)
    ...
    if signal_type == "BUY":
        ...
        if candle_bullish and strong_move:
            return True, "✅ 1H candle confirming BUY — bullish close with strong body"
        elif candle_bullish and not_ranging:
            return True, "✅ 1H candle bullish — moderate confirmation"
        else:
            return False, "❌ 1H candle NOT confirming BUY — wait for bullish 1H close"
        ...
        if candle_bearish and strong_move:
            return True, "✅ 1H candle confirming SELL — bearish close with strong body"
        elif candle_bearish and not_ranging:
            return True, "✅ 1H candle bearish — moderate confirmation"
        else:
            return False, "❌ 1H candle NOT confirming SELL — wait for bearish 1H close"
...
def build_plain_english_explanation(signal_type, trend, fvg, bpr, sr, sd, fib, sweep_detected, sweep_msg, h1_msg, struct_msg, instrument):
    ...
    # 1H confirmation
    lines.append(f"5️⃣  1H CANDLE: {h1_msg}")
    lines.append(f"   → Switch your MT5 chart to the 1 HOUR timeframe")
    if signal_type == "BUY":
        lines.append(f"   → You should see the current 1H candle closing as a BULLISH (green) candle")
    else:
        lines.append(f"   → You should see the current 1H candle closing as a BEARISH (red) candle")
    lines.append("")
...
def generate_signal(name, symbol):
    ...
    print(f"      🕯️  Checking 1H candle confirmation...")
    h1_confirmed, h1_msg = check_1h_candle_confirmation(symbol, signal_type)
    if not h1_confirmed:
